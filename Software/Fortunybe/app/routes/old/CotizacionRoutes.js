...
module.exports = function (app) {
  app.route('/Cotizacion/Export/:id')
    .get(function (req, res) {
      // req.file is the `avatar` file
      var templatePDF = "cotizacionPDF"
      CotizacionController().getPrint(req.params.id, req.query.showExWork, function (status, result) {
  ...
  app.route('/Cotizacion/Export/confirmacion/:id')
    .get(function (req, res) {
      var templatePDF = "confirmacionPDF"
      CotizacionController().getPrint(req.params.id, req.query.showExWork, function (status, result) {
        console.log("datoss")
        console.log(result[0].pagina[0].NumCotizacion)
        var nombrePdf = (result[0].pagina[0].NumCotizacion).split("/")
        nombrePdf = nombrePdf[0] + nombrePdf[1]
        if (status == 200) {
          exportPDF(status, result, templatePDF, nombrePdf, function (status, result) {
  ...
  app.route('/Cotizacion/:id')
    ...
    , function (req, res) {
      CotizacionController().update(req.userId,req.rolId, req.tenant, req.params.id, req.body, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
    .delete(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().delete(req.tenant, req.params.id, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
    .get(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().getbyId(req, res, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    });
  app.route('/CotizacionesNoCerradas')
    .get(function (req, res) {
      CotizacionController().getAllNotClosed(req.userId, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
  app.route('/Cotizacion')
    .get(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().getAll(req.tenant, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
    .post(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().create(req.userId, req.tenant, req.body, res, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    });
  ...
  app.route('/CotizacionRechazaCliente/:id')
    .put(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().clienteRechaza(req.userId,req.rolId, req.tenant, req.params.id, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
  app.route('/CotizacionAceptaCliente/:id')
    .put(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().clienteAcepta(req.tenant, req.userId,req.rolId, req.params.id, req.body, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
  ...
  app.route('/CancelarCotizacion/:id')
    .put(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().CancelarCotizacion(req.tenant, req.userId, req.rolId, req.params.id, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
  app.route('/ordenConfirmacion/:id')
    .put(function (req, res, next) { validateRolAccess(req, res, next) }
    , function (req, res) {
      CotizacionController().confirmacionOrden(req.userId, req.tenant, req.params.id, req.body, function (statusCode, result) {
        res.status(statusCode).json(result);
      });
    })
}
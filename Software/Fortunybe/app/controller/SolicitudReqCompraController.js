...
module.exports = function () {
				...
				function (err, result) {
					if (err) {	
						...
						if (result && result != "") {
							...
							cb(400, { texto: "SOLICITUD ERRONEA O YA RESPONDIDA" });
						}
					}
				}
			);
	...
	/*function createRequest(userid, datos, cb) {
	...
	return {
		...
		create: function(userid, datos, cb) {
			...
			var budgetOK = true;
			if (budgetOK){
				//Update Estatus and asign to user
				...
				/*}*/
				//Notify
				/*var objNot = {titulo: 'Nuevo Solicitud de Compra', descripcion: 'Se ha creado la solicitud ', link:'#/reqCompra/detalle/' , usuario_id : 38}
				notificacion().create(objNot, function (code, message){
					if (code != 200) {
	                    cb(code, message);
	                } else {
		                 cb(200, {});
